...
$('#gen').onclick = async () => {
  const btn = $('#gen'); btn.disabled = true; btn.textContent = 'Summoning…'; $('#ai-note').textContent = 'The AI is drawing your dungeon. This can take up to a minute.';
  try {
    const r = await api('/api/levels/generate', { method: 'POST', body: { prompt: $('#prompt').value, difficulty: Number($('#diff').value), size: $('#size').value } });
