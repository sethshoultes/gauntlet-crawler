...
// committed under client/audio/voice/ — speechSynthesis is still the fallback for the remaining
// stable ids that have no clip yet (`amulet_invis`, `amulet_reflect`, `amulet_repulse`,
// `amulet_super`, `boost_pickup`, `you_are_it`), for the per-scene `cutscene` caption (whose text
// differs every time, so it's never pre-rendered), and if a shipped clip ever fails to load.
// Every line runs through the voice mixer volume (see client/audio.js's sibling master/SFX buses
// — voice deliberately has no WebAudio graph of its own since both playback paths, HTMLAudio and
