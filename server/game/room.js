...
export class Room {
  ...
  info() {
    return {
      ...
      roster: [...this.clients.values()].map((c) => ({ name: c.name, cls: c.cls })),
    };
  }
  ...
  join(ws, { pid, user, name, cls }) {
    ...
    const c = { ws, pid, user, name, cls, joinedAt: Date.now(), streak: 0 };
    this.clients.set(pid, c);
    this.emptySince = null;
    this.sim.addPlayer(pid, { name, cls, userId: user?.id || null });
  ...
  onEvent(e) {
    ...
    switch (e.type) {
      case 'kill': bump('kills'); bump(`kills_${e.monster}`); break;
      case 'generator': bump('generators'); break;
      case 'food': bump('food'); if (e.lowHealth) bump('food_low'); break;
      case 'food_shot': bump('food_shot'); break;
      case 'pickup': if (e.item === 'T') bump('treasure'); if (e.item === 'K') bump('keys'); break;
